import './capsuleB.css';


function CapsuleB() {
  return (
    <div id='capsuleB_container'>
      <div id='capsuleB_title'>
        <h3>캡슐커피 즐기는 방법</h3>
      </div>
      <div id='capsuleB_main'>
        <div id='capsuleB_a'>
          <div id='capsuleB_aa'>
            <div id='capsuleB_img1'>
              <img src='/capsuleimg/beanist_step01_01.gif'></img>
            </div>
            <div>
              <p>추출된 20g 에스프레소에 뜨거운 물 60ml를 가수하세요.</p>
              <p>(1:3 비율)</p>
            </div>
            <div id='capsuleB_img2'>
              <img src='/capsuleimg/beanist_step02_02.gif'></img>
            </div>
          </div>
        </div>
        <div id='capsuleB_arrow'>
          <h1>›</h1>
        </div>
        <div id='capsuleB_a'>
          <div id='capsuleB_aa'>
            <div id='capsuleB_img1'>
              <img src='/capsuleimg/beanist_step02_01.gif'></img>
            </div>
            <div>
              <p>기호에 따라 우유, 크림 및 시럽 등을</p>
              <p>첨가하여 다양한 라떼를 즐기세요.</p>
            </div>
            <div id='capsuleB_img2'>
              <img src='/capsuleimg/beanist_step03_02.gif'></img>
            </div>
          </div>
        </div>
      </div>
      <div id='capsuleB_bot'>
        <div id='capsuleB_title'>
          <h3>이디야 캡슐커피 특징</h3>
        </div>
        <div id='capsuleB_bot_a'>
          <div id='capsuleB_img_bot'>
            <img src='/capsuleimg/capsule_pro_img.jpg'></img>
          </div>
          <div id='capsuleB_bot_aa'>
            <p>이디야 캡슐커피는 커피의 맛을 좌우하는 맛과 향을 최대한 지키기 위해 특허 받은 아로마 실링을 사용한 제품입니다.
              보존성이 가장 뛰어난 EVOH 플라스틱의 이중 캡슐구조로 매장에서 갓 내린 에스프레소 맛을 즐기실 수 있습니다.</p>
            <h4>*아로마실링팩은 향을 지키기 위한 특수 소재의 포장재입니다.</h4>
          </div>
        </div>
      </div>
    </div>
  );
}

export default CapsuleB;
